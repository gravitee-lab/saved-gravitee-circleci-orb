...
## Bash-Getting the maven project version using the maven exec plugin

* One of the main technicalpoints into the `mvn_release.sh` script, is to retireve, from the `pom.xml`, in a bash sript, the maven project's version.
* The maven exec plugin allowsto do that nicely, :

![Bash-Getting the maven project version using the maven exec plugin](./mvn_exec_plugin_n_project_version.png)

## ANNEX : Circle CI Orbs File Inclusion Feature
